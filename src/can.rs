//! CAN typ 1 interface of stm32f103xx
//! stm32f100xx, stm32f101xx, stm32f102xx do not have CAN
//! stm32f105xx, stm32f107xx has CAN typ 2, which is not supported yet
//!
...
pub type FilterBankIndex = usize; //[0..FILTERBANKCOUNT-1] is valid
    ...
    pub fn configure_filter_bank(
        ...
        match config.info {
            FilterInfo::Halves((ref low, ref high)) => {
                ...
                let fbi = &self.can.fb[index];
                fbi.fr1.write(|w| unsafe { w.bits(low.id.as_16bit_filter() | (low.mask_or_id2.as_16bit_filter() << 16)) });
                fbi.fr2.write(|w| unsafe { w.bits(high.id.as_16bit_filter() | (high.mask_or_id2.as_16bit_filter() << 16)) });
            }
            FilterInfo::Whole(ref whole) => {
                ...
                let fbi = &self.can.fb[index];
                fbi.fr1.write(|w| unsafe{ w.bits(whole.id.as_32bit_filter()) });
                fbi.fr2.write(|w| unsafe{ w.bits(whole.mask_or_id2.as_32bit_filter()) });
            }
        };
    ...
    ($CANX:ident, [
        ...
        $(
            ...
            impl TransmitMailbox for TxMailBox<$CANX, $TxMailBoxi>
            {
                ...
                //     unsafe { (*$CANX::ptr()).$tdtir.read().time().bits() }
                // }
                ...
                fn request_transmit(&mut self, frame: &Frame) -> Result<(), Error> {
                    if self.is_empty() {
                        ...
                        let txi = unsafe { &((*$CANX::ptr()).tx[$i]) };

                        //fill message length [0..8]
                        txi.tdtr.write(|w| unsafe { w.dlc().bits(frame.data.dlc) });
                        txi.tdlr.write(|w| unsafe { w.bits(frame.data.data_low) }); 
                        txi.tdhr.write(|w| unsafe { w.bits(frame.data.data_high) }); 
            ...
            impl ReceiveFifo for RxFifo<$CANX, $RxFifoi> {
                ...
                fn read(&mut self) -> nb::Result<(FilterMatchIndex, TimeStamp, Frame), ()> {
                    ...
                    if n < 1 {
                        ...
                        let rxi = unsafe { &(*$CANX::ptr()).rx[$i] };
                        let rdtir = rxi.rdtr.read();
                        let filter_match_index = rdtir.fmi().bits();
                        let time = rdtir.time().bits();

                        let frame = Frame {
                            id: Id::from_received_register(rxi.rir.read().bits()),
                            data: Payload {
                                dlc: rdtir.dlc().bits(),
                                data_low: rxi.rdlr.read().bits(),
                                data_high: rxi.rdhr.read().bits(),
                            },
                        };
